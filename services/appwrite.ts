...
export const updateSearchCount = async (query: string, movie: Movie) => {
  try {
    const result = await database.listDocuments(DB_ID, COLLECTION_ID, [Query.equal('searchTerm', query)]);

    if(result.documents.length) {
      const existingMovie = result.documents[0];

      await database.updateDocument(DB_ID, COLLECTION_ID, existingMovie.$id, { count: existingMovie.count + 1 });
    } else {
